// ==UserScript==
// @name         RU-Board (MV3)
// @description  Статусы онлайн, поиск сообщений, скрытие шапки, цитирование (адаптация для Tampermonkey MV3)
// @date         22.09.2025
// @author       Halibut/Wald + Artyom Shegeda/greeple/Capushon + MV3 fix by ChatGPT
// @version      1.3-mv3
...
this.whosonline = ({
    ...
    load: function () {
        ...
        try {
            ...
            // Старый IE нам не нужен, оставлено на всякий
        ...
        this.xmlHttp.onreadystatechange = function () {
            if (self.xmlHttp && self.xmlHttp.readyState == 4) {
    ...
    highlightUsers: function () {
        ...
        try {
            ...
            for (var i = 0; i < elements.length; i++) {
                ...
                if (elements[i].parentNode && elements[i].parentNode.className == 'm' && this.users[elements[i].innerText]) {
                ...
                else if (elements[i].parentNode && elements[i].parentNode.className == 'm') {
            ...
            // не спамим alert-ами
            console.error('[RU-Board MV3] highlightUsers error:', e);
        }
    },
    run: function () {
        return this;
...
whosonline && (whosonline.load(), window.addEventListener('load', function () {
...
window.addEventListener("DOMContentLoaded", function f() {
    ...
    const body = document.body,
          ...
          listener = {
              options: {
                  ...
                  autoOpenSpoilerWithResult: false,
                  reversPostSorting: true,
                  headToResult: false,
                  srchInSigs: false,
                  srchInQuotes: false,
                  imgsToLinks: true,
                  createCnMenu: true,
                  searchOnProfileLinksByRMB: true,
                  fillSearchFormWithNames: true,
                  nameToFillSrchForm: null,
                  namesToSearch: ['Name1', 'Name2', 'Name3', 'etc...'],
                  highlightMatches: false,
                  highlightColor: 'yellow'
              },
              names: [],
              xhrPool: [], // тут будут AbortController
              txt: '',
              dtStmp: null,
              ...
              get name() { return this.name = this.getName() },
              set name(str) { return this.setName(str) },
              get text() { const text = this.txt; delete this.txt; return text; },
              set text(str) { if (str) return this.txt = str; },
              get win() { delete this.win; return this.win = window.top },
              get loc() { delete this.loc; return this.loc = this.win.location.href },
              get actnBox() {
                  delete this.actnBox;
                  return this.actnBox = [...document.getElementsByTagName("table")]
                    .find(el => el.querySelector("a[href^='post.cgi?action=new']") || !el.getElementsByTagName('td')[0].children.length)
              },
              get isFF() {
                  delete this.isFF;
              ...
              get spHd() { delete this.spHd; return this.spHd = this.getSpHd() },
              get spBd() { delete this.spBd; return this.spBd = this.getSpBd() },
              get spTTl() { delete this.spTTl; return this.spTTl = this.spHd && this.spHd.getElementsByTagName('td')[0] },
              get navBox() { delete this.navBox; return this.navBox = this.getNavBox() },
              getSel() { return this.win.getSelection && this.win.getSelection().toString() || "" },
              abortXhrs() {
                  if (!this.xhrPool.length) return;
                  for (const ctrl of this.xhrPool) ctrl && ctrl.abort();
              ...
              prepareSntc(str, cs, ww) {
                  ...
                  if (ww) return wwrgxp(escStr(str));
                  else return rgxp(escStr(str));
              },
              getCrntPost(arr) {
              ...
              prompt(name, txt) {
                  return new Promise((res, rej) => {
                      ...
                      imgs.onclick = () => { this.options.imgsToLinks = imgs.checked }
                      highlight.checked = this.options.highlightMatches;
                      highlight.onclick = () => { this.options.highlightMatches = highlight.checked }
                      sig.checked = this.options.srchInSigs;
                      sig.onclick = () => { this.options.srchInSigs = sig.checked }
                      qt.checked = this.options.srchInQuotes;
                      qt.onclick = () => { this.options.srchInQuotes = qt.checked }
                      ...
                      body.onwheel = e => { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); };
                      ...
                      fromNew.onclick = fromOld.onclick = () => { this.options.reversPostSorting = fromNew.checked; }
                      regChkbx.onclick = e => {
              ...
              // MV3: заменено на fetch + DOMParser + AbortController
              getPosts([url, page], name, txt, stmp) {
                  return new Promise((res, rej) => {
                      if (this.dtStmp === null) { res(null); return; }
                      else if (this.dtStmp !== stmp) { rej([null, stmp]); return; }
                      ...
                      const rmv = itm => {
                          const i = this.xhrPool.indexOf(itm);
                          if (i !== -1) this.xhrPool.splice(i, 1);
                      };
                      this.xhrPool.push(controller);

                      fetch(url, { signal: controller.signal, credentials: 'include', cache: 'no-store' })
                        .then(r => {
                            if (!r.ok) throw new Error(String(r.status || 'fetch_error'));
                            return r.text();
                        })
                        .then(html => {
                            if (this.dtStmp !== null && this.dtStmp !== stmp) { rej([null, stmp]); return; }
                            const doc = new DOMParser().parseFromString(html, 'text/html');
                            let p = doc && doc.getElementsByClassName('tb');
                            if (p && p.length) {
                                p = this.filterPosts([...p], name, txt, page);
                                res(p);
                            } else {
                                res(null);
                            }
              ...
              filterPosts(posts, name, txt, page) {
                  return posts.filter(post => {
                      const nameCell = post.querySelector("td.dats b");
                      const userNameNorm = nameCell ? nameCell.textContent.toLowerCase().replace(/\s/g, '_') : '';
                      const isUser = name && userNameNorm && name.includes(userNameNorm);
                      const chkHead = this.options.headToResult && page == 1 || !post.querySelector('a.tpc[href$="&postno=1"]');
                      const isText = () => {
                          const pstEl = post.getElementsByClassName('post')[0];
                          const highligthMatches = () => {
                          ...
                          if (txt.exclude) {
                              ...
                              if (isCntsExcludes) return false;
                              else if (!txt.val) return true;
                          }
                          switch (txt.type) {
                              case 'all': return txt.val.every(sntnc => sntnc.test(pstTxt)) && highligthMatches(pstEl);
                              case 'rgxp':
                              case 'snt': return txt.val.test(pstTxt) && highligthMatches(pstEl);
                              case 'some':
                              case 'any': return (Array.isArray(txt.val) ? txt.val.some(sntnc => sntnc.test(pstTxt)) : txt.val.test(pstTxt)) && highligthMatches(pstEl);
                          }
                      };
              ...
              flatPosts(posts) { return posts.reduce((a, b) => a.concat(b), []) },
              imgsToLinks(posts) {
              ...
              setName(str) { if (str) return this.names.push(str); },
                      ...
                      const spoilerHead = this.spHd,
                            ...
                            crntPst = () => {
                                const posts = [...nodes].slice(0, -1),
                                      possY = posts.map(el => el.getClientRects()[0]?.y ?? 0);
              ...
              handleEvent(e, name, prmpt) {
                  ...
                  this.spawn(function*() {
                      ...
                      if (spoilerBody.children && !!spoilerBody.children.length)
                          for (const node of [...spoilerBody.children]) node.remove();
          ...
          highlightHlp = 'Включает подсветку совпадений в тексте найденных постов. Может влиять на производительность и верстку.',
    ...
    if (dats)
        for (const dat of dats) {
            const br = dat.appendChild(document.createElement("br"));
            const button = dat.appendChild(document.createElement("button"));
            const name = dat.getElementsByTagName("b")[0]?.textContent || '';
            ...
            button.addEventListener('click', e => listener.handleEvent(e, name, false));
            button.addEventListener('contextmenu', e => listener.handleEvent(e, name, true));
        }

    if (listener.options.searchOnProfileLinksByRMB)
        body.addEventListener("contextmenu", e => {
            const anchor = e.target && (e.target.closest && e.target.closest("a[href*='?action=show&member=']"));
            const name = anchor && anchor.search && anchor.search.split("&member=")[1];
            if (!anchor || !name || e.altKey || e.shiftKey || e.ctrlKey || e.metaKey
                || !!listener.getSel() || anchor.closest("#spu-spoiler-body")) return;
            listener.handleEvent(e, name, false);
        });
    ...
    if (listener.isFF && listener.options.createCnMenu) {
        ...
        mitem.label = "Найти сообщения пользователя(-ей)";
...
document.addEventListener ("DOMContentLoaded", function DOM_ContentReady () {
    "use strict";
    document.removeEventListener("DOMContentLoaded", DOM_ContentReady);
    ...
    (function () {
        ...
        for (var i = 0; i < TrT.length; i++) {
            ...
            if (TdTag.length >= 3) {
                if (TdTag[2].className == "tpc") {
                    // создаём ссылку без inline-обработчиков
                    var link = document.createElement('a');
                    link.className = 'tpc';
                    link.title = 'Вставка цитаты с именем автора';
                    link.textContent = 'Цитата';
                    link.href = '#';

                    // Вставляем "Цитата | " в начало ячейки
                    TdTag[2].insertBefore(document.createTextNode(' | '), TdTag[2].firstChild);
                    TdTag[2].insertBefore(link, TdTag[2].firstChild);
        ...
        function InsertSel1() {
            ...
            function FindPrnt1(aNode) {
                ...
                var PrNd1 = aNode.parentNode || aNode;
                if (!PrNd1 || !PrNd1.tagName) {
                    return PrNd1;
                }
                if (PrNd1.tagName.toLowerCase() == 'table') {
                    if (PrNd1.className == "tb") {
                        ...
                        if (BTag.length != 0) {
                            LastFindUserName = BTag[0].textContent || BTag[0].innerText || '';
                            return PrNd1;
                        }
                    }
                }
            ...
            function FindRef1(TrNode) {
                ...
                for (var i = ATg1.length - 1; i >= 0; i--) {
                    var href = ATg1[i].href || '';
                    if (href.indexOf('topic.cgi') >= 0) {
                        if ((LastFindUserName != '') && href) {
                            var s1 ='[url=' + href + '][b]' + LastFindUserName + '[/b][/url]';
                            return s1;
                        }
                    }
                }
            ...
            if (!sel || sel.rangeCount === 0) {
                alert('Сначала выделите текст сообщения, которое хотите процитировать.');
                return;
            }
            var range = sel.getRangeAt(0);
            var Nd = FindPrnt1(range.startContainer);
            if (!Nd || !Nd.tagName) {
                alert('Тег не найден.');
            } else {
                var s1 = FindRef1(Nd);
                if (s1 == '') {
                    ...
                    var s2 = sel.toString() || ''; // вместо txt/copyQ()
                    var TATag1 = document.getElementById('post');
                    if (TATag1 == null) {
                        alert('Форма ответа не найдена.');
                    } else {
                        var start = TATag1.selectionStart || 0;
                        var end = TATag1.selectionEnd || 0;
                        TATag1.value = TATag1.value.substr(0, start) + s1 + s2 + TATag1.value.substr(end);
                        // опционально: поставить курсор в конец вставленного блока
                        var pos = (TATag1.value.substr(0, start) + s1 + s2).length;
                        if (TATag1.setSelectionRange) {
                            TATag1.focus();
                            TATag1.setSelectionRange(pos, pos);
                        }
                    }
                }
            }
        ...
        // Отключаем стандартное цитирование (без глобала insert_window)
